...
const defaultInstrumentParams = TRACKS_IDS.reduce<InstrumentParams>(
  (
    acc: InstrumentParams,
    curr,
  ) => (acc[curr] = { ...defaultInstrument }, acc as InstrumentParams),
  {},
);
    ...
    persist(
      subscribeWithSelector(
        (set, get) => ({
          ...
          changeBars: (bars: number) =>
            set((state) => {
              const newActive = getNewBars(state.activeBars, bars);
              return {
                activeBars: newActive,
                scheduledEvents: removeEventsFromInactiveBars(
                  newActive,
                  state.scheduledEvents,
                ),
              };
            }),
          changeTracks: (tracks: number) =>
            set((state) => ({
              ...
              (state) => (
                {
                  trackSettings: {
                    ...
                    [id]: {
                      ...
                      mute: !state.trackSettings[id].mute,
                      solo: false,
                    },
                  },
                }
              ...
              (state) => {
                if (state.trackSettings[id].solo) {
                  return {
                    trackSettings: {
                      ...state.trackSettings,
                      [id]: { ...state.trackSettings[id], solo: false },
                    },
                  };
                }
              ...
              (state) => (
                {
                  trackSettings: {
                    ...state.trackSettings,
                    [id]: { ...state.trackSettings[id], volume: vol },
                  },
                }
              ),
              false,
          ...
          removeTriggersForTrack: (instrumentId: number) =>
            set((state) => {
              return {
                scheduledEvents: state.scheduledEvents.filter((trigger) =>
                  TriggersService.parseTrigger(trigger).instrumentId !==
          ...
          duplicateBarEvents: () =>
            set((state) => {
              return {
                scheduledEvents: getEventsWithDuplicates(
                  state.activeBars,
                  state.scheduledEvents,
                ),
...
function removeEventsFromInactiveBars(
  activeBars: number,
  scheduledEvents: Array<string>,
) {
...
function getNewTracks(activeTracks: number, change: number): number {
  ...
  return (newTracks > 0 && newTracks <= InstrumentsService.instruments.length)
    ? newTracks
    : activeTracks;
}
